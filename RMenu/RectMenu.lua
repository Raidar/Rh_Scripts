...
function VisCount.Base (Len, Sep, Total, Base, Fixes) --> (number, number)
  ...
  --logMsg({ k, Total, Sep, L, T, Len, Base, sum, Fixes }, "Base", 2, "#q")
  ...
  --logMsg({ k, Total, Sep, L, sum, Fixes }, "Base", 2, "#q")
...
function VisCount.Pale (Len, Sep, Total, Pale, Fixes) --> (number, number)
  ...
  --logMsg({ k, Total, Sep, L, T, Len, Pale, Fixes }, "Pale", 2, "q")
  repeat
    L = L + Len[k] + Sep
    k = k - 1
  until k < Count or L > T -- end
  ...
  --logMsg({ k, Total, Sep, L, T, Len, Pale, sum, Fixes }, "Pale", 2, "#q")
  ...
  --logMsg({ k, Total, Sep, L, sum, Fixes }, "Pale", 2, "#q")
...
function TMenu:Show (DlgProc)
  ...
  --logMsg({ self.List[self.SelIndex], self.Menu.Items[SelIndex] }, SelIndex, 1)
...
function TMenu:DefinePropInfo () --| Props
  ...
  --logMsg(self.Colors, "self.Colors", 2, "#h2")
end ---- DefinePropInfo
...
function TMenu:DefineDataInfo () --| Data
  ...
  --logMsg({ Data.Count, Idx2Cell(SelIndex, Data) }, SelIndex)
...
function TMenu:DefineSpotInfo () --| Zone
  ...
  local function textLen (Item, Index, Selex, Col) --> (bool)
    ...
    if not Item.RectMenu.MultiLine then
      ...
      --if not ss then logMsg(Item, Index, 1) end
  ...
  --logMsg({ self.textMax, self.lineMax, self.ColWidth, self.RowHeight }, "Spot", 2, "#q")
end ---- DefineSpotInfo
...
function TMenu:DefineZoneInfo () --| Zone
  ...
  --logMsg({ self.FixedRows, self.FixedRows }, "Fixed")
...
function TMenu:DefineDBoxInfo () --| (Dlg...)
  ...
  --logMsg({ Titles, Titles.Top:len(), Titles.Bottom:len(),
  ...
  --logMsg(self.DlgPos, "Dlg Pos")

  -- Задание формы окна диалога меню.
  self.Form = self:DialogForm()
  --logMsg(self.Form, "Form table", 2, "ft_")
end ---- DefineDBoxInfo
...
function TMenu:UpdateAll (hDlg, Flags, Table) --| (self)
  ...
  if isRedraw == nil then
    --logMsg({ oldR, newR }, "UpdateAll", 2, "#q")
  ...
  --logMsg({ Flags, Pos, { isUpdate, isRedraw, isRedrawAll } }, "UpdateAll")
...
function TMenu:SetSelected (SelIndex)
  ...
  --logMsg(self.List, "SelIndex  = "..tostring(self.SelIndex), 1)
end ----
...
function TMenu:FitSelected (SelIndex) --> (number)
  ...
  --logMsg({ k, self.Data.Count, { Idx2Cell(k, self.Data) } }, "FitSelected")
...
function TMenu:WrapCell (Row, Col, dRow, dCol) --> (Cell)
  ...
  if dRow ~= 0 then -- Ячейка по строкам:
    --logMsg({ Cell, RC_cell(Row, Col) }, "dRow ~= 0")
    if outrange(Row, Fixes.Min, Fixes.Max) then
      Cell = { Row = Row }
    else _, Cell = self:FindRowCell(Row, Col, dRow) end
    --logMsg({ Cell, RC_cell(Row, Col), { Fixes.Min, Fixes.Max } }, "dRow ~= 0")
    if Cell then
      Row = Cell.Row -- Поиск:
      if Row > Fixes.Max or
         (Row == Fixes.Max and self:CellIndex(Row, Col) == 0) then
        Row = Fixes.Min - 1
      elseif Row < Fixes.Min then Row = Fixes.Max + 1 end
    else
      if dRow > 0 then Row = Fixes.Min - 1
      elseif dRow < 0 then Row = Fixes.Max + 1 end
    end
  end -- if
  ...
  if dCol ~= 0 then -- Ячейка по столбцам:
    if outrange(Col, Fixes.Min, Fixes.Max) then
      Cell = { Col = Col }
    else _, Cell = self:FindColCell(Row, Col, dCol) end
    if Cell then
      Col = Cell.Col -- Поиск:
      if Col > Fixes.Max or
         (Col == Fixes.Max and self:CellIndex(Row, Col) == 0) then
        Col = Fixes.Min - 1
      elseif Col < Fixes.Min then Col = Fixes.Max + 1 end
    else
      if dCol > 0 then Col = Fixes.Min - 1
      elseif dCol < 0 then Col = Fixes.Max + 1 end
    end
  end -- if
  --logMsg(RC_cell(Row, Col), "WrapCell")
...
function TMenu:GotoCell (aCell, dCell, isNew) --> (SelIndex, Cell)
  ...
  --logMsg({ aCell, dCell, isNew }, "GotoCell")
  ...
  --logMsg({ RC_cell(R, C), RC_cell(dR, dC) }, "FindCell")
  local Index, Cell = self:FindCell(R, C, dR, dC) -- Прямо и обратно!
  --logMsg(Cell, "FindCell: "..tostring(self:isIndex(Index)))
...
function TMenu:CellBase (aCell) --| Zone.Base
  ...
  if Cell.Row < Base.Row then
    ...
    Cat = Cell.Row - Cat + 1
    --logMsg({ Base, Pale, Cell }, "Row: "..tostring(Row))
  ...
  if Cell.Col < Base.Col then
    ...
    Cat = Cell.Col - Cat + 1
    --logMsg({ Base, Pale, Cell }, "Col: "..tostring(Col))
    if Cat > Base.Col then Base.Col = min2(Cat, Pale.Col) end
  end
  --logMsg({ Base, Cell }, "Cells")
end ---- CellBase
...
function TMenu:MoveToCell (hDlg, NewIndexCell) --> (bool)
  ...
  --logMsg({ OldIndex, Base, oCell, nCell, dCell }, NewIndex, "#q")
...
function TMenu:GetArrowCell (oCell, aKey) --> (values)
  ...
  --]]
  --logMsg({ oCell, VisRows, VisCols, Last }, "Arrow Cell Data")
...
function TMenu:ChooseItem (hDlg, Kind, Index) --> (nil|boolean)
  ...
  --logMsg({ Result, Table, Flags }, "OnKeyPress", 2)
...
function TMenu:RapidKeyPress (hDlg, VirKey) --> (bool)
  ...
  if ( RM.AltHotOnly and (IsModAlt(VMod) or IsModAltShift(VMod)) ) or
    ...
    --logMsg({ VirKey, StrKey, self.HKeys }, "HKeys : "..tostring(SKey), 1, "hv8")
    if SKey and not UnhotSKeys[SKey] then
      local Index = self.HKeys:cfind(SKey, 1, true)
      local VName = VirKey.KeyName
      --logMsg({ Index, VName, self.HKeys }, tostring(SKey), 1, "hv8")
      ...
      --logMsg(self.HKeys, "HKeys : "..tostring(SKey).." at "..tostring(Index), 1, "hv8")
      if Index then
        self:SetSelected(Index)
  ...
  --logMsg({ Index, self.BKeys }, StrKey, 1, "h8")
  if Index then
    self.KeyIndex = Index
...
function TMenu:UserKeyPress (hDlg, VirKey) --> (nil|true | Table)
  ...
  --logMsg({ VirKey, Table, Flags }, "OnKeyPress", 2)
...
function TMenu:DoKeyPress (hDlg, VirKey) --> (bool)
  --logMsg(self, "self", 1, "#q")
  local SelIndex = self.SelIndex
  --logMsg(VirKey, SelIndex, 1, "h8")
...
function TMenu:ScrollVClick (hDlg, pos)
  ...
  local function ScrollVCell (OldCell) --> (table, number, table)
    ...
    --logMsg({ OldCell, bCell, dCell, Bar, FixRows }, pos)
    return dCell, self:GotoCell(bCell, dCell, true)
  end --
...
function TMenu:DrawMenuPart (A_Cell, A_Rect, A_Colors)
  ...
  if y < yLim then
    ...
    --logMsg(Rect, "Draw Spaces by y", 1, "#q")
    DrawClearItemText(Rect, A_Colors.COL_MENUTEXT, Spaces)
  end -- if
  --logMsg({ x, xLim }, "Values")
...
function TMenu:DrawMenu ()
  ...
  --logMsg({ xMin, xMax, yMin, yMax }, "DrawMenuList", 1, "#q")
  ...
  --logMsg({ FixRows, FixCols }, "Fixed", 1, "#q")
...
local function DrawTitleLine (X, Y, TitleColor, Title, Color, Line, Show)
  ...
  --logMsg({ Title, X, Y, Len, Width, LineLen, Width - LineLen - Len })
...
function TMenu:DrawBorderLine () -- --| Border
  ...
  --logMsg({ X1, Y1, X2, Y2, W, H }, "DrawBorderLine")
...
local function ScrollCaret (ALen, Cat, Count) --> (Pos, Len, End)
  ...
  --logMsg({ ALen, Cat, Count, '-------', Len, Pos, Pos + Len - 1 }, "ScrollCaret")
...
function TMenu:DrawScrollBars ()
  ...
  --logMsg({ Zone, Cell }, "Zone and Cell", 1, "#q")

  -- Сведения о полосах прокрутки меню.
  self:CalcScrollBars()
  --logMsg(self.ScrollBars, "self.ScrollBars")
...
local function Menu (Properties, Items, BreakKeys, ShowMenu) --> (Item, Pos)
  ...
  --logMsg(BreakKeys, "BreakKeys", 2, "#q")
...
--[[ 1. Конфигурирование меню ]]
  ...
  --logMsg(_Menu, "_Menu", 1, "#q")

  -- Определение вида меню:
  _Menu:DefineAll()
  --logMsg(_Menu, "_Menu", 1, "#qt")
...
--[[ 2.2. Обработка реакции меню ]]
  ...
  local function DlgCtrlEvent (hDlg, ProcItem, Input) --> (bool)
    ...
    if StrKey == "Enter" or StrKey == "NumEnter" then -- Выбор пункта:
      --logMsg(_Menu.List[_Menu.SelIndex], _Menu.SelIndex)
